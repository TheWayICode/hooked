...
function HudsonRiverDetails() {
  ...
  const fishList = async () => {
    const fishUrl = `${process.env.REACT_APP_USER_SERVICE_API_HOST}/api/locations/4`;
    ...
    if (fishResponse.ok) {
      ...
      const map = new mapboxgl.Map({
        ...
        center: [-73.79993546808437, 42.550187229084955],
        zoom: 12,
      });
      ...
      <div className="w-full bg-cover bg-white bg-no-repeat py-10 px-4">
        <div className="max-w-[1240px] mx-auto grid md:grid-cols-">
          <img
            ...
            src="https://cdn.pixabay.com/photo/2019/10/10/23/48/landscape-4540748_960_720.jpg"
            alt="SMP"
            style={{ width: "100%" }}
          />
          ...
          <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded shadow">
            <a
              href={`https://www.google.com/maps/dir/?api=1&destination=${42.550187229084955},${-73.79993546808437}`}
              className="mb-4"
            >
              Get directions
            </a>
...
export default HudsonRiverDetails;
